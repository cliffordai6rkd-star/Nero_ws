        ...
        if self.config.predictor.enabled:
            ...
            if control_mode == "q":
                ...
                if q_target is None or dq_target is None or tau_target is None:
                    raise RuntimeError("contact WM MIT mode requires q/dq/tau targets")
                ...
                self.arm.command_joint_impedance(
                    ...
                    kp=kp,
                    kd=kd,
                    # Firmware applies the PD terms; t_ff is WM feedforward only.
                    t_ff=np.asarray(tau_target, dtype=np.float64),
                )
            else:
                # Legacy OSC-QP, contact OSC-QP, and direct-tau execution all
                # use the pure-torque MIT transport.
