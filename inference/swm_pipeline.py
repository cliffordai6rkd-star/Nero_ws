"""Inference pipeline for the native PINN torque state world model (SWM).

The SWM checkpoint has a deliberately different contract from the historical
contact world model: its condition is ``q``, ``dq``, ``delta_q``, ``tau`` plus
an 8-token direct joint action and mask.  This module keeps that contract
isolated while reusing the mature DP observation/action scheduling in
...
class SWMInferencePipeline(NeroInferencePipeline):
    """Run the q/dq/delta_q/tau torque world model at its training rate."""

    def __init__(self, config, **kwargs) -> None:
        if not bool(config.predictor.enabled):
            raise ValueError("SWMInferencePipeline requires predictor.enabled=true")
        if str(config.action).strip().lower() != "joint":
            raise ValueError(
                "SWM inference requires action: joint (the checkpoint action is "
                "direct absolute joint position)"
            )
        mode = str(config.predictor.mode).strip().lower().replace("-", "_")
        if mode not in _SWM_MODES:
            raise ValueError(
                "SWMInferencePipeline requires predictor.mode=swm or swm_opd"
            )
        super().__init__(config, **kwargs)

        model = self.pinn
        state_contract = str(getattr(model, "state_contract", "")).lower()
        if state_contract and state_contract != "q_dq_delta_q_tau":
            raise ValueError(
                "SWM checkpoint must expose state_contract='q_dq_delta_q_tau'"
            )
        supported = tuple(getattr(model, "SUPPORTED_INPUTS", ()))
        if supported and any(key not in supported for key in ("q", "dq", "delta_q", "tau")):
            raise ValueError("SWM model.SUPPORTED_INPUTS is missing a required state stream")
        ...
        if configured_inputs and not {"q", "dq", "delta_q", "tau"}.issubset(
            {str(key).lower() for key in configured_inputs}
        ):
            raise ValueError(
                "SWM checkpoint model.inputs must include q, dq, delta_q, and tau"
            )
        action_dim = int(
            getattr(
                model,
        ...
        self._swm_action_start_offset = int(
            data_cfg.get("action_start_offset", 1)
        ) if isinstance(data_cfg, Mapping) else 1
        if self._swm_action_start_offset < 0:
            raise ValueError("SWM dataloader.action_start_offset must be non-negative")
        ...
        # remainder is the exact sequence consumed by the SWM condition.
        expert_fps = data_cfg.get("expert_fps", 25.0) if isinstance(data_cfg, Mapping) else 25.0
        self._swm_expert_fps = float(expert_fps)
        if not np.isfinite(self._swm_expert_fps) or self._swm_expert_fps <= 0.0:
            raise ValueError("SWM dataloader.expert_fps must be positive and finite")
        self._swm_action_period_s = 1.0 / self._swm_expert_fps
        if min(self._swm_history_horizon, self._swm_future_horizon, self._swm_action_horizon) < 1:
            raise ValueError("SWM horizons must be positive")
        self._swm_history = {
            key: deque(maxlen=self._swm_history_horizon)
            for key in ("q", "dq", "delta_q", "tau")
        }
        self._swm_previous: tuple[float, dict[str, np.ndarray]] | None = None
        ...
        self._swm_last_q_cmd: np.ndarray | None = None
        high_fps = (
        ...
        if configured_dt is None and isinstance(loss_cfg, Mapping):
            # SWM's smoothness/kinematic losses use this same physical state
            # period; prefer it when the checkpoint records it explicitly.
            configured_dt = loss_cfg.get("dt")
        self._swm_sampling_dt_s = (
            float(configured_dt)
            if configured_dt is not None
            else (float(1.0 / float(high_fps)) if high_fps else 0.01)
        )
        if not np.isfinite(self._swm_sampling_dt_s) or self._swm_sampling_dt_s <= 0:
            self._swm_sampling_dt_s = 0.01
        self._swm_next_sample_s: float | None = None
        self._swm_action_key = str(model_cfg.get("action_key", "action"))
        flow_steps = getattr(
            model,
            "flow_inference_steps",
            model_cfg.get("flow_inference_steps", 8),
        )
        self._swm_flow_steps = int(flow_steps or 8)
        self._swm_flow_solver = str(
            getattr(model, "flow_solver", model_cfg.get("flow_solver", "heun"))
        ).lower()
        if self._swm_flow_steps < 1:
            raise ValueError("SWM flow inference steps must be positive")
        ...
        if dp_action_start is not None and int(dp_action_start) != self._swm_action_start_offset:
            raise ValueError(
                "SWM action_start_offset must match the DP checkpoint action_start_index: "
                f"swm={self._swm_action_start_offset} dp={int(dp_action_start)}"
            )

    def _validate_action_cadence(self) -> None:
        """Require the high-level action clock to match SWM training."""

        expected = float(self._swm_action_period_s)
        ...
        if configured is not None and abs(float(configured) - expected) > tolerance:
            raise ValueError(
                "SWM predictor.action_step_s must match checkpoint expert_fps: "
        ...
        # models and older checkpoints, which then use the SWM cadence below.
        dp_step = self.observation_step_s
        if dp_step is not None and abs(float(dp_step) - expected) > tolerance:
            raise ValueError(
                "SWM requires DP action cadence to match checkpoint expert_fps: "
                f"dp={float(dp_step):.9g}s expected={expected:.9g}s"
            )

    def _action_execution_step_s(self) -> float:
        """Advance DP/SWM action chunks at the checkpoint's expert rate."""

        return float(self._swm_action_period_s)

    def reset(self) -> None:
        super().reset()
        for values in self._swm_history.values():
            values.clear()
        self._swm_previous = None
        self._swm_last_q_cmd = None
        self._swm_next_sample_s = None

    # Keep the inherited continuous-observation bridge, but replace its old
    # q/v/a/wrench state with the four streams used by SWM training.
    def _append_world_model_observation(self, sample: InferenceInput) -> None:
        ...
        if q_cmd is None:
            ...
            # For offline/injected callers, reuse the previous SWM command;
            # never substitute the DP action because it is only a condition and
            # is not necessarily what the low-level transport applied.
            q_cmd = self._swm_last_q_cmd
        if q_cmd is None:
            q_cmd = sample.q
        self._append_swm_observation_values(
    ...
    def _append_world_model_observation_values(
        ...
        if any(key not in values for key in required) or ("dq" not in values and "v" not in values):
            raise ValueError("SWM observation requires q, dq, and tau")
    ...
    def _append_swm_observation_values(
        self,
        timestamp_s: float,
        q: Any,
        dq: Any,
        ...
        if not np.isfinite(timestamp_s):
            raise ValueError("SWM observation timestamp must be finite")
        vectors = {
        ...
        if any(
            ...
            raise ValueError("SWM q/dq/delta_q/tau must be finite seven-vectors")
        previous = self._swm_previous
        if previous is None:
            self._append_swm_values(vectors)
            self._swm_previous = (timestamp_s, vectors)
            self._swm_next_sample_s = timestamp_s + self._swm_sampling_dt_s
            return
        ...
        if timestamp_s <= previous_s:
            if abs(timestamp_s - previous_s) <= timestamp_tolerance_s:
                return
            raise ValueError("SWM observation timestamps must increase")
        assert self._swm_next_sample_s is not None
        while self._swm_next_sample_s <= timestamp_s + 1.0e-9:
            alpha = (self._swm_next_sample_s - previous_s) / (timestamp_s - previous_s)
            interpolated = {
            ...
            self._append_swm_values(interpolated)
            self._swm_next_sample_s += self._swm_sampling_dt_s
        self._swm_previous = (timestamp_s, vectors)

    def _append_swm_values(self, values: Mapping[str, np.ndarray]) -> None:
        for key, value in values.items():
            history = self._swm_history[key]
    ...
    def step(self, sample: InferenceInput) -> InferenceOutput:
        ...
        reference = self._predict_swm_reference()
        ...
        if self._dp_action_chunk is None:
            ...
            output = self._idle_swm_output(sample, dp_updated)
        else:
            output = self._execute_swm_reference(
                sample, current_control_pose, reference, dp_updated
            )
        self._remember_swm_command(sample, output)
        self._timing_cycles += 1
        self._report_timing(perf_counter() - cycle_started)
        return output

    def _remember_swm_command(
        ...
        if value.shape == (7,) and np.all(np.isfinite(value)):
            self._swm_last_q_cmd = value.copy()

    def _predict_swm_reference(self) -> dict[str, Any]:
        import torch

        if any(len(values) < 1 for values in self._swm_history.values()):
            raise RuntimeError("SWM state history is empty")
        device = _model_device(self.pinn)
        history = {
            key: np.stack(tuple(values), axis=0) for key, values in self._swm_history.items()
        }
        inputs = {
        ...
        actions, mask = self._swm_action_condition(device)
        ...
        with torch.inference_mode():
            try:
                output = self.pinn.predict(
                    inputs,
                    steps=self._swm_flow_steps,
                    solver=self._swm_flow_solver,
                )
            except TypeError:
        ...
        if not isinstance(output, Mapping) or not isinstance(output.get("state_pred"), Mapping):
            raise RuntimeError("SWM predict() must return a state_pred mapping")
        state_pred = output["state_pred"]
        result: dict[str, Any] = {}
        for key in ("q", "dq", "delta_q", "tau"):
            value = state_pred.get(key)
            if value is None:
                raise RuntimeError(f"SWM state_pred is missing {key!r}")
            if not torch.is_tensor(value):
                value = torch.as_tensor(value, dtype=torch.float32, device=device)
            if value.ndim != 3 or value.shape[0] != 1 or value.shape[-1] != 7:
                raise RuntimeError(
                    f"SWM state_pred[{key!r}] must have shape [1,T,7], got {tuple(value.shape)}"
                )
            if value.shape[1] != self._swm_future_horizon:
                raise RuntimeError(
                    f"SWM state_pred[{key!r}] horizon {value.shape[1]} != {self._swm_future_horizon}"
                )
            ...
            if not torch.isfinite(physical).all():
                raise RuntimeError(f"SWM state_pred[{key!r}] is non-finite")
        ...
        if contact is not None:
            ...
            if contact.shape != (self._swm_future_horizon, 1):
                raise RuntimeError(
                    "SWM contact_state must have shape "
                    f"[{self._swm_future_horizon}, 1], got {contact.shape}"
                )
            if not np.isfinite(contact).all():
                raise RuntimeError("SWM contact_state is non-finite")
            result["contact_state"] = contact
        return result

    def _swm_action_condition(self, device):
        import torch
        ...
        if source.ndim != 2 or source.shape[1] != 7:
            raise RuntimeError(f"SWM direct action must be [H,7], got {source.shape}")
        valid = min(source.shape[0], self._swm_action_horizon)
        values = np.repeat(source[-1:, :], self._swm_action_horizon, axis=0)
        ...
        # action is available.
        mask = np.ones(self._swm_action_horizon, dtype=np.float32)
        return (
    ...
    def _idle_swm_output(self, sample: InferenceInput, dp_updated: bool) -> InferenceOutput:
        ...
        common = dict(
            ...
            contact_state=None,
            control_mode=self._swm_execution_mode,
    ...
    def _safe_joint_position_trajectory(
        ...
        *,
        ...
        if targets.ndim != 2 or targets.shape[1] != 7 or targets.shape[0] < 1:
            raise ValueError(f"SWM joint position trajectory must be [H,7], got {targets.shape}")
        if not np.all(np.isfinite(targets)):
            raise ValueError("SWM joint position trajectory must be finite")
        dt_s = float(dt_s)
        if not np.isfinite(dt_s) or dt_s <= 0.0:
            raise ValueError("SWM sampling dt must be positive and finite")
...
TorqueWorldModelInferencePipeline = SWMInferencePipeline
SWMPipeline = SWMInferencePipeline
